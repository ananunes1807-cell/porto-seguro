...
function gerarRelatorio(evento){
  ...
  const dados={identificacao:identificacaoRelatorio(lista,profissionalTipo,profissionalNome),clinico:informacoesClinicasDeclaradas(),objetivo:$('#relatorio-objetivo').value.trim()||'Não informado.',estadoAtual:estadoAtual(lista),mudancas:mudancasPercebidas(lista),gatilhos:gatilhosContextos(lista),risco:detectarRisco(lista),estrategias:estrategiasUtilizadas(lista),protecao:fatoresProtecao(lista),impactos:impactosCotidiano(lista),recorrentes:temasMaisRecorrentes(lista,profissionalTipo),pontosConsulta:pontosConsulta(lista,$('#observacoes-relatorio').value.trim(),profissionalTipo)};

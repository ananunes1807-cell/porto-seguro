...
function frasesTemas(temas){return temas.map(t=>`${t.categoria}: mencionado(a) em ${t.contagem} de ${t.total} registros selecionados.`)}
...
// Reproduz literalmente o que a pessoa escreveu no Perfil de Acolhimento —
// nunca sugere, confirma ou interpreta um diagnóstico.
function informacoesClinicasDeclaradas(){const diagnosticos=estado.perfil?.diagnosticos?.trim(),medicacoes=estado.perfil?.medicacoes?.trim();if(!diagnosticos&&!medicacoes)return'Nenhum diagnóstico ou medicação em uso foi informado no Perfil de Acolhimento.';const linhas=[];if(diagnosticos)linhas.push(`Diagnóstico(s) informado(s) pela pessoa: ${diagnosticos}`);if(medicacoes)linhas.push(`Medicação(ões) em uso informada(s) pela pessoa: ${medicacoes}`);return linhas.join('\n')}
...
const PERGUNTA_EXTRA_POR_PROFISSIONAL={
  'Psicólogo(a)':'Quais emoções, pensamentos ou situações mencionadas nos registros merecem ser explorados nas próximas sessões?',
  'Psiquiatra':'Há aspectos relacionados às medicações em uso ou a ajustes relatados pela pessoa que merecem ser avaliados pelo profissional em relação às mudanças percebidas no período?',
  'Médico(a)':'Há aspectos clínicos que podem ajudar a compreender os sintomas físicos, alterações de sono ou apetite relatados?',
  'Terapeuta ocupacional':'Quais rotinas, ambientes ou adaptações poderiam reduzir a sobrecarga percebida no cotidiano?',
  'Assistente social':'Existem redes de apoio, recursos comunitários ou encaminhamentos que poderiam ser úteis neste momento?'
};
function pontosConsulta(lista,observacoes,profissionalTipo){const perguntas=['Como a pessoa está se sentindo em relação ao período descrito neste relatório?'];const temasMudancas=tabularTemas(lista,MAPA_MUDANCAS);if(temasMudancas.length)perguntas.push(`Há aspectos relacionados a ${temasMudancas[0].categoria}, mencionado(a) nos registros, que a pessoa gostaria de discutir na consulta?`);const temasGatilhos=tabularTemas(lista,MAPA_GATILHOS);if(temasGatilhos.length)perguntas.push(`Como lidar com os episódios relacionados a ${temasGatilhos[0].categoria.toLowerCase()}?`);const extra=PERGUNTA_EXTRA_POR_PROFISSIONAL[profissionalTipo];if(extra)perguntas.push(extra);perguntas.push('As estratégias já tentadas pela pessoa têm sido úteis? Há outras que poderiam ajudar?','Necessita de avaliação profissional adicional em relação aos temas trazidos neste relatório?');let texto=perguntas.map((p,i)=>`${i+1}. ${p}`).join('\n');if(observacoes)texto+=`\n\nObservações adicionais informadas pela pessoa:\n${observacoes}`;return texto}

const OBSERVACAO_EXTRA_POR_PROFISSIONAL={
  'Psiquiatra':'Os registros do período também mencionam sono, energia e medicações informadas pela pessoa.',
  'Médico(a)':'Os registros do período também mencionam sintomas físicos e alterações percebidas.',
  'Terapeuta ocupacional':'Os registros do período também mencionam impacto em rotina e autonomia.',
  'Assistente social':'Os registros do período também mencionam rede de apoio e recursos relatados.'
};
// Reaproveita as mesmas tabulações já usadas nas seções de mudanças, gatilhos
// e impactos (nenhuma lista de palavras-chave nova) — só reordena por
// frequência. Nunca recomenda técnica de intervenção nem indica prioridade
// clínica: a frase final deixa isso explicitamente a cargo da profissional.
function temasMaisRecorrentes(lista,profissionalTipo){const candidatos=[...tabularTemas(lista,MAPA_MUDANCAS),...tabularTemas(lista,MAPA_GATILHOS),...tabularTemas(lista,MAPA_IMPACTOS)].sort((a,b)=>b.contagem-a.contagem).slice(0,3);if(!candidatos.length)return'Os registros selecionados não indicam temas recorrentes claros.';const linhas=candidatos.map(c=>`${c.categoria}: mencionado(a) em ${c.contagem} de ${c.total} registros selecionados.`);const extra=OBSERVACAO_EXTRA_POR_PROFISSIONAL[profissionalTipo];if(extra)linhas.push(extra);linhas.push('A relevância clínica e eventual priorização destes temas devem ser avaliadas pelo profissional responsável.');return linhas.join('\n')}

// A seção de risco nunca é aceita de uma reescrita externa, sob nenhuma condição.
// Uma versão anterior tentava comparar o texto original com a reescrita por
// presença de palavras-chave (ex.: "plano", "intenção") para permitir a reescrita
// só quando os mesmos achados apareciam nos dois — mas a frase padrão de recusa
// ("não fornecem informações suficientes para determinar a presença de plano,
// intenção...") já contém essas mesmas palavras negando-as, então uma resposta
// fabricada que afirmasse "a pessoa tem um plano definido e intenção" passava
// pela checagem sem ser detectada. Comparação de texto livre não é confiável o
// bastante para essa seção; por isso a seção de risco é sempre o texto local,
// nunca a versão colada, sem exceção.
// Extrai, por código (nunca por IA), a frase original do registro que bateu
// a palavra-chave de uma categoria de risco — verbatim, sem paráfrase, sem
// risco de "pensei em" virar "tentou". Só é chamada quando a pessoa marcou
// "Incluir detalhes íntimos"; sem isso, a seção continua só com categoria e
// contagem, como antes.
function trechoDeRisco(lista,achadosCategoria,palavras){
  const id=[...achadosCategoria][0];if(!id)return null;
  const registro=lista.find(r=>r.id===id);if(!registro)return null;
  const texto=registroTextoBusca(registro),frases=texto.split(/(?<=[.!?\n])\s+/).map(f=>f.trim()).filter(Boolean);
  const frase=frases.find(f=>palavras.some(p=>normalizarTexto(f).includes(p)))||texto;
  const palavrasFrase=frase.trim().split(/\s+/).filter(Boolean),maxPalavras=30;
  const trecho=palavrasFrase.slice(0,maxPalavras).join(' ')+(palavrasFrase.length>maxPalavras?'…':'');
  return trecho?`Trecho registrado: "${trecho}" (${formatarData(registro.createdAt)}).`:null;
}
function detectarRisco(lista,incluirTrechos){const achados={};Object.keys(MAPA_RISCO).forEach(chave=>achados[chave]=new Set());lista.forEach(r=>{const texto=normalizarTexto(registroTextoBusca(r));Object.entries(MAPA_RISCO).forEach(([chave,palavras])=>{if(palavras.some(p=>texto.includes(p)))achados[chave].add(r.id)})});const rotulos={pensamentosDeMorte:'Pensamentos de morte',autolesao:'Autolesão',medicacaoAlemPrescrito:'Vontade de tomar medicação além do prescrito',atendimentoHospitalar:'Atendimento hospitalar por crise'};const linhas=[];let algumaMencao=false;const adicionar=(chave,rotulo)=>{if(!achados[chave].size)return false;linhas.push(rotulo);if(incluirTrechos){const trecho=trechoDeRisco(lista,achados[chave],MAPA_RISCO[chave]);if(trecho)linhas.push(trecho)}return true};['pensamentosDeMorte','autolesao','medicacaoAlemPrescrito','atendimentoHospitalar'].forEach(chave=>{if(adicionar(chave,`${rotulos[chave]}: mencionado(a) em ${achados[chave].size} registro(s).`))algumaMencao=true});const temPlano=achados.plano.size>0,temIntencao=achados.intencao.size>0,temMeios=achados.acessoAMeios.size>0;if(adicionar('plano',`Plano: mencionado em ${achados.plano.size} registro(s).`))algumaMencao=true;if(adicionar('intencao',`Intenção: mencionada em ${achados.intencao.size} registro(s).`))algumaMencao=true;if(adicionar('acessoAMeios',`Acesso a meios: mencionado em ${achados.acessoAMeios.size} registro(s).`))algumaMencao=true;if(!temPlano&&!temIntencao&&!temMeios)linhas.push('Os registros não fornecem informações suficientes para determinar a presença de plano, intenção ou acesso atual a meios. Recomenda-se avaliação direta pela profissional.');if(!algumaMencao)linhas.unshift('Nenhuma menção explícita a pensamentos de morte, autolesão ou atendimento hospitalar por crise foi identificada nos registros selecionados.');return linhas.join('\n')}

function montarRelatorioResumido(dados){const secoesAtivas=new Set($$('.secao-relatorio-toggle:checked').map(x=>x.dataset.secao)),blocos=['RELATÓRIO RESUMIDO PARA ACOMPANHAMENTO','','1. IDENTIFICAÇÃO',dados.identificacao];if(secoesAtivas.has('clinico'))blocos.push('','2. INFORMAÇÕES CLÍNICAS DECLARADAS PELA PESSOA',dados.clinico);if(secoesAtivas.has('objetivo'))blocos.push('','3. OBJETIVO INFORMADO PELA PESSOA',dados.objetivo);if(secoesAtivas.has('estadoAtual'))blocos.push('','4. ESTADO ATUAL',dados.estadoAtual);if(secoesAtivas.has('mudancas'))blocos.push('','5. MUDANÇAS PERCEBIDAS NO PERÍODO',dados.mudancas);if(secoesAtivas.has('gatilhos'))blocos.push('','6. POSSÍVEIS GATILHOS E CONTEXTOS',dados.gatilhos);blocos.push('','7. SITUAÇÕES DE RISCO',dados.risco);if(secoesAtivas.has('estrategias'))blocos.push('','8. ESTRATÉGIAS UTILIZADAS',dados.estrategias);if(secoesAtivas.has('protecao'))blocos.push('','9. RECURSOS E FATORES DE PROTEÇÃO IDENTIFICADOS NOS REGISTROS',dados.protecao);if(secoesAtivas.has('impactos'))blocos.push('','10. IMPACTOS NO COTIDIANO',dados.impactos);if(secoesAtivas.has('recorrentes'))blocos.push('','11. TEMAS MAIS RECORRENTES NOS REGISTROS',dados.recorrentes);if(secoesAtivas.has('consulta'))blocos.push('','12. QUESTÕES PARA DISCUSSÃO COM O PROFISSIONAL',dados.pontosConsulta);blocos.push('','13. LIMITAÇÕES','Este documento organiza informações selecionadas pela própria pessoa. Não constitui diagnóstico, laudo, prontuário ou avaliação profissional.');return blocos.join('\n')}
...
function gerarRelatorio(evento){
  ...
  const dados={identificacao:identificacaoRelatorio(lista,profissionalTipo,profissionalNome),clinico:informacoesClinicasDeclaradas(),objetivo:$('#relatorio-objetivo').value.trim()||'Não informado.',estadoAtual:estadoAtual(lista),mudancas:mudancasPercebidas(lista),gatilhos:gatilhosContextos(lista),risco:detectarRisco(lista,$('#relatorio-incluir-detalhes-intimos').checked),estrategias:estrategiasUtilizadas(lista),protecao:fatoresProtecao(lista),impactos:impactosCotidiano(lista),recorrentes:temasMaisRecorrentes(lista,profissionalTipo),pontosConsulta:pontosConsulta(lista,$('#observacoes-relatorio').value.trim(),profissionalTipo)};
...
function cancelarRelatorio(){estado.relatorioAtual=null;estado.dadosRelatorioAtual=null;$('#texto-relatorio-editavel').value='';$('#contador-palavras-relatorio').textContent='';$('#resultado-relatorio').hidden=true}
...
// --- Prompt pronto para colar em qualquer IA (gratuito, sem backend) -------
// "identificacao" (nome da pessoa) e "clinico" (diagnóstico/medicação declarados)
// nunca vão para o prompt de IA por padrão — são dados sensíveis demais para
// sair do aparelho sem que a pessoa perceba. Se um dia isso mudar, deve exigir
// uma opção separada e explícita, nunca fazer parte do envio padrão.
function payloadSecoesIA(dados){const{identificacao,clinico,...resto}=dados;return resto}
const INSTRUCOES_PROMPT_IA=`Você vai reescrever seções de um relatório de acompanhamento emocional em prosa profissional e neutra, em português do Brasil, para uma pessoa apresentar a um(a) profissional de psicologia, psiquiatria, medicina, terapia ocupacional ou assistência social.

Regras obrigatórias, sem nenhuma exceção:
- Nunca diagnostique e nunca afirme relação de causa e efeito que não esteja explícita nos dados abaixo. Nunca complete lacunas por inferência.
- Comece frases preferencialmente com: "A pessoa relata", "Foi registrado", "A pessoa percebe", "Segundo o relato", "Nos registros selecionados" — ou variações neutras equivalentes. Não escreva uma percepção subjetiva da pessoa como se fosse fato objetivo (ex.: se a pessoa escreveu "ninguém se importa comigo", não escreva "ninguém oferece apoio à pessoa" — escreva "a pessoa relata sentir que não recebe apoio").
- Use SOMENTE os dados abaixo, já resumidos localmente. Nunca invente fatos, nomes, datas ou números que não estejam aqui.
- Preserve a história, não só as categorias: quando os dados trouxerem um acontecimento, contexto ou experiência concreta (não só "categoria: mencionado(a) em N de M registros"), mantenha essa informação de forma resumida e fiel, em vez de reduzir tudo à categoria isolada. Prefira "Foram registrados conflitos conjugais relacionados a mudanças de planos, promessas não cumpridas e sensação de não poder contar com o parceiro" a apenas "Conflitos conjugais foram mencionados".
- Quando uma expressão curta da própria pessoa ajudar a compreender sua experiência, você pode preservá-la entre aspas, deixando claro que é um relato dela (ex.: "descreve, em suas próprias palavras, sentir-se 'como um objeto para os outros'"). Não corrija o sentido, não aumente nem suavize a gravidade da expressão — pode corrigir só erros gramaticais quando necessário para compreensão.
- Se os dados mostrarem momentos diferentes dentro do período (ex.: sofrimento intenso em um momento e retomada de atividades em outro), preserve essa oscilação em vez de resumir o período inteiro como uma tendência única — só quando ambos os elementos estiverem presentes nos dados.
...
- O campo "risco" é só contexto: já contém a decisão final sobre situações de risco, calculada de forma determinística, sem IA, e será mantido exatamente como está não importa o que você escreva nele. Não tente reescrevê-lo; pode devolvê-lo sem mudanças ou omiti-lo da resposta.
...
- Responda SOMENTE com um objeto JSON válido, com exatamente as mesmas chaves recebidas abaixo, sem markdown, sem comentários, sem texto fora do JSON.

Dados:`;
function montarPromptIA(payload){return`${INSTRUCOES_PROMPT_IA}\n${JSON.stringify(payload,null,2)}`}
function abrirPromptIA(){if(!estado.dadosRelatorioAtual)return $('#status-relatorio').textContent='Gere o relatório antes de criar o prompt para IA.';const payload=payloadSecoesIA(estado.dadosRelatorioAtual);$('#texto-prompt-ia').value=montarPromptIA(payload);$('#resposta-ia-colada').value='';$('#status-prompt-ia').textContent='';abrirModal($('#tela-prompt-ia'))}
async function copiarPromptIA(){const texto=$('#texto-prompt-ia').value;try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(texto);else{const el=$('#texto-prompt-ia');el.select();document.execCommand('copy')}$('#status-prompt-ia').textContent='Prompt copiado. Cole em uma IA de sua confiança.'}catch{$('#status-prompt-ia').textContent='Não foi possível copiar automaticamente. Selecione o texto e copie manualmente.'}}
function aplicarRespostaIA(){
  const bruto=$('#resposta-ia-colada').value.trim();
  if(!bruto)return $('#status-prompt-ia').textContent='Cole a resposta da IA antes de aplicar.';
  if(!estado.dadosRelatorioAtual)return;
  let json;
  try{json=JSON.parse(bruto.replace(/^```(?:json)?\s*|```$/g,'').trim())}
  catch{return $('#status-prompt-ia').textContent='Não consegui interpretar a resposta como JSON. Copie exatamente o que a IA respondeu, sem texto extra antes ou depois.'}
  if(!json||typeof json!=='object'||Array.isArray(json))return $('#status-prompt-ia').textContent='A resposta colada não é um objeto JSON válido.';
  const payloadOriginal=payloadSecoesIA(estado.dadosRelatorioAtual),secoesRevisadas={};
  for(const chave of Object.keys(payloadOriginal))secoesRevisadas[chave]=typeof json[chave]==='string'&&json[chave].trim()?json[chave].trim():payloadOriginal[chave];
  secoesRevisadas.risco=payloadOriginal.risco; // nunca aceito de fora, sob nenhuma condição — ver comentário no início desta seção
  const dadosFinal={...estado.dadosRelatorioAtual,...secoesRevisadas};
  let texto=montarRelatorioResumido(dadosFinal);
  if($('#relatorio-incluir-citacoes').checked){const lista=estado.registros.filter(r=>estado.relatorioAtual?.registros?.includes(r.id));const citacoes=montarCitacoes(lista,!$('#relatorio-incluir-nomes-terceiros').checked);if(citacoes)texto+=`\n\n===== CITAÇÕES BREVES DOS REGISTROS (opcional) =====\n${citacoes}`}
  $('#texto-relatorio-editavel').value=texto;
  atualizarContadorPalavras();
  fecharModal($('#tela-prompt-ia'));
  $('#status-relatorio').textContent='Texto aprimorado por IA aplicado. Revise com atenção antes de salvar ou imprimir.';
}
